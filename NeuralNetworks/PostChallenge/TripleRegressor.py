...
class TripleRegressor:
   ...
   def trainme (self, input_explanatory, input_response, input_valset, input_training, normalization_params, num_epochs=10000, earlystop_epochsNoChange=100, size_minibatch=100, datasplit = { "percent_train": 0.80 }):
      ...
      shuf_uniqrows_training = np.array (input_training);

      indexes_train = []
      indexes_validation = []
      indexed_ignored = [] # never use indexes that exist in OUTTER loop validation set
      for index, row in dfinput_ExplanatoryVariables.iterrows():
         ...
         elif (np.array(row) == shuf_uniqrows_training).all(1).any() == True:
            indexes_train.append (index)
         else:
            indexed_ignored.append (index)

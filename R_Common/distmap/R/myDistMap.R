...
setGeneric(name = "my.binarizeSingleCellData",
           def = function(object, quantiles=seq(0.15, 0.16, 0.01), cells_train) {
               standardGeneric("my.binarizeSingleCellData")
           })
setMethod(f = "my.binarizeSingleCellData",
          signature = "DistMap",
          function(object, quantiles, cells_train) {
              ...
              # Use only training cells to find "best.quantile"
              data2train <- object@data[, cells_train]
              
              for (quantile in quantiles) {
                  gene.thresholds <- sapply(insitu.genes, function(gene){
                      quantile(as.numeric(data2train[gene, data2train[gene, ] > 0]), quantile)
                  })
                  names(gene.thresholds) <- gsub("\\..*", "", names(gene.thresholds))
                  binarized.data <- data2train[insitu.genes, ]
                  binarized.data[binarized.data >= 0] <- 0
                  binarized.data[apply(data2train[insitu.genes, ], 2, function(cell) cell > gene.thresholds)] <- 1
              ...
              # Use only training cells to find "best.gene.thresholds"
              best.gene.thresholds <- sapply(insitu.genes, function(gene) {
                  quantile(x = as.numeric(data2train[gene, data2train[gene, ] > 0]),
                           probs = best.quantile)})
              names(best.gene.thresholds) <- gsub("\\..*", "", names(best.gene.thresholds))
              
              # Binarize all cells 
              # - Note: Each cell is binarized independently from the rest
...
setGeneric(name = "my.mapCells",
           def = function(object) {
               standardGeneric("my.mapCells")})
setMethod(f = "my.mapCells",
          signature = "DistMap",
          function(object) {
              mcc.scores.list <- mclapply(1:(dim(object@data)[2]), function(cell) computeMCC(object, cell), 
                                          mc.cores = detectCores())
              mcc.scores <- do.call(what = cbind, mcc.scores.list)
